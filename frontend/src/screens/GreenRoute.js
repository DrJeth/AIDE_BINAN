...
const GOOGLE_MAPS_API_KEY = 'AIzaSyA6NOYNehpIXFNVrLoPLloZ6fcajBb9kns'; // Add your API key here
...
export default function GreenRoute() {
  ...
  const backendReverse = async (lat, lon) => {
    ...
    try {
      ...
      const r2 = await fetch(url2, { headers: { 'User-Agent': 'GreenRouteApp' }});
  ...
  const nominatimSearchFallback = async (query) => {
    ...
    try {
      ...
      const resp = await fetch(url, { headers: { 'User-Agent': 'GreenRouteApp' }});
      const j = await resp.json();
  ...
  // NEW: Fetch directions from Google Maps Directions API
  const fetchGoogleDirections = async (origin, destination) => {
    try {
      ...
      if (json.status === 'OK' && json.routes && json.routes.length > 0) {
        ...
        return { coords, steps: allSteps };
      } else {
        Alert.alert('Directions Error', json.error_message || 'Unable to fetch directions');
        return null;
      }
    } catch (e) {
      console.warn('fetchGoogleDirections error', e);
      Alert.alert('Error', 'Failed to fetch Google directions');
      return null;
    }
  };
    ...
    try {
      ...
      if (havePolygon) {
        ...
        if (!isPointInPolygon(dest.latitude, dest.longitude, cityOutline)) {
          Alert.alert('Destination outside Biñan', 'Point B is outside Biñan city.');
          setLoading(false);
          return;
        }
      }
      ...
      if (result) {
        setTimeout(() => {
          if (!mapRef.current || result.coords.length === 0) return;
  ...
  onPress={() => this.props.navigation.navigate("HomeRider")}
>
